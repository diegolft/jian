...
function setup() { const gateway = new Gateway(new MemoryStore()); const app = createApp({ gateway, token, logger: false }); apps.push(app); return { app, gateway }; }
...
describe('HTTP gateway', () => {
  ...
  it('creates profiles and sessions, then queues idempotent runs', async () => {
    const { app } = setup();
    const created = await app.inject({ method: 'POST', url: '/v1/profiles', headers, payload: input });
    expect(created.statusCode).toBe(201);
    const id = created.json().id;
    const session = await app.inject({ method: 'POST', url: `/v1/profiles/${id}/sessions`, headers, payload: { title: 'Mac', channel: 'macos' } });
    const url = `/v1/profiles/${id}/sessions/${session.json().id}/messages`;
    const first = await app.inject({ method: 'POST', url, headers, payload: { text: 'Hello', requestKey: 'same' } });
    const duplicate = await app.inject({ method: 'POST', url, headers, payload: { text: 'Hello', requestKey: 'same' } });
  ...
  it('rejects secret values and unknown settings without echoing payloads', async () => {
    const { app } = setup();
    const response = await app.inject({ method: 'POST', url: '/v1/profiles', headers, payload: { ...input, model: { ...input.model, apiKey: 'secret-123' } } });
  ...
  it('returns explicit conflicts and replayable event cursors', async () => {
    ...
    await app.inject({ method: 'PATCH', url, headers, payload: { expectedVersion: 1, name: 'Updated' } });
    const stale = await app.inject({ method: 'PATCH', url, headers, payload: { expectedVersion: 1, name: 'Stale' } });
    expect(stale.statusCode).toBe(409);
    const events = (await app.inject({ url: `${url}/events`, headers })).json();
    expect(events).toHaveLength(2);
    const resumed = (await app.inject({ url: `${url}/events?after=${events[0].id}`, headers })).json();
...
it('streams committed events over HTTP and resumes after a cursor', async () => {
  ...
  try {
    const response = await fetch(`${url}/v1/profiles/${profile.id}/events/stream`, { headers: { ...headers, 'Last-Event-ID': String(initial.id) }, signal: controller.signal });
