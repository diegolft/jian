...
export class MemoryStore implements Store {
  ...
  async get<K extends Kind>(kind: K, id: string): Promise<Records[K] | null> {
    return structuredClone(
      (this.records.get(`${kind}:${id}`)?.data as Records[K] | undefined) ?? null,
    );
  }

  async list<K extends Kind>(kind: K, options: Selection = {}): Promise<Records[K][]> {
    let rows = [...this.records.values()]
      .filter((r) => r.kind === kind && (!options.profileId || r.profileId === options.profileId))
      .map((r) => r.data as Records[K]);

    if (options.before) {
      const cursor = rows.findIndex((row) => row.id === options.before);

      rows = cursor < 0 ? [] : rows.slice(0, cursor);
    }
    ...
    if (options.anyWords?.length) {
      rows = rows.filter(
        (row) =>
          'content' in row &&
          options.anyWords?.some((word) =>
            `${'key' in row ? row.key : ''} ${row.content}`
              .toLowerCase()
    ...
    if (options.search) {
      ...
      rows = rows.filter(
        (row) =>
          'content' in row &&
          terms.every((term) =>
            `${'key' in row ? row.key : ''} ${row.content}`.toLowerCase().includes(term),
          ),
      );
    }
  ...
  async events(profileId: string, after: number, limit = 100) {
    return structuredClone(
      this.log.filter((e) => e.profileId === profileId && e.id > after).slice(0, limit),
    );
  }

  async transaction<T>(_profileId: string, body: (tx: Transaction) => Promise<T>): Promise<T> {
    const execute = async () => {
      const old = structuredClone(this.records);
      const oldLog = structuredClone(this.log);
